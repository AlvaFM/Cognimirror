// DigitSpanMirror.tsx - El Sensor Verbal (Digit Span Test) - v2.0
// Con voz española, números visuales y teclado físico
...
export const DigitSpanMirror = ({ userId, userName, onBack, onGameComplete }: DigitSpanMirrorProps) => {
  ...
  const handleDigitInput = (digit: string) => {
    ...
    if (newInput.length === sequence.length) {
      setTimeout(() => checkAnswer(newInput, [...currentRoundTaps, tapData]), 500);
    }
  };
              ...
              <div className="text-center space-y-2 w-full max-w-sm">
                ...
                <div className="grid grid-cols-3 gap-1.5 max-w-[240px] mx-auto">
                  {[1, 2, 3, 4, 5, 6, 7, 8, 9, null, 0, null].map((digit, index) => {
                    if (digit === null) {
                      return <div key={`empty-${index}`} className="w-20 h-20" />;
                    }
                    return (
                      <button
                        key={digit}
