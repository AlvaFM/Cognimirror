...
export const VoiceOnboardingWelcome = ({ onNavigate }: VoiceOnboardingWelcomeProps) => {
  ...
  const handleTraditionalSubmit = async (e: React.FormEvent) => {
    ...
    // LÓGICA COPIADA DE MISIÓN 2: Firestore
    try {
      ...
      if (existingUser) {
        setError('el nombre de usuario esta ocupado, selecciona otro');
        return;
      }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Validar nombre duplicado
      const existingUser = querySnapshot.docs.find(doc =>
        doc.data().nombre?.toLowerCase() === finalName.trim().toLowerCase()
      );

      if (existingUser) {
        addToHistory('❌ Error: el nombre de usuario esta ocupado');
        return;
      }
      ...
      console.log('✅ Guardando usuario en Firestore:', finalName.trim(), finalAge);
      addToHistory(`✅ Guardado en Firestore: ${finalName.trim()}, ${finalAge} años (ID: ${nextId})`);

      // 4. Establecer como paciente seleccionado con el ID correcto de Firestore
      setUserWithFirestoreId(nextId, finalName.trim(), finalAge);

      // 5. Navegar al perfil del usuario
      onNavigate('patient-profile');
    } catch (err) {
      console.error('Error al crear usuario:', err);
      addToHistory('❌ Error al crear usuario en Firestore');
    }
  };
  ...
  if (mode === 'vocal') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden">
        ...
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-full shadow-2xl transition-all duration-300 ${isListening
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 max-h-64 overflow-y-auto mb-6">
              ...
              <div className="space-y-2">
                {stateHistory.map((message, index) => (
                  <div
                    key={index}
                    className={`p-3 rounded-lg ${message.startsWith('🤖')
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-all ${listeningField === 'name'
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  disabled={listeningField !== null}
                  className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-all ${listeningField === 'age'
                    ? 'bg-red-500 text-white animate-pulse'
                    : 'bg-gray-100 text-gray-600 hover:bg-blue-100 hover:text-blue-600'
                    } disabled:opacity-50 disabled:cursor-not-allowed`}
                  title="Dictar edad"
